...
- 对接 ONLYOFFICE Document Server，浏览器内直接编辑 docx / xlsx / pptx
...
- 允许 = 仅查看（不勾下载）。预览接口只要求"查看"权限，
  下载与打包接口要求"下载"权限，所以这个设定是真的生效的。
...
可保存回原格式的：`docx` `xlsx` `pptx` `odt` `ods` `odp` `txt` `csv`。
