...
- 批量上传自动排队（同时最多 3 个文件），断线自动重试
- 撞上同名文件时像 Windows 那样先问一句：替换 / 保留两者 / 跳过
...
POST   /upload/complete           合并分片（幂等：重复调用返回同一个文件）
                                  # /upload 与 /upload/init 都收一个 conflict 参数：
                                  # rename(默认，改名) / replace(覆盖，需编辑权限) / skip(重名则返回 409)
...
**一次传几十个文件，为什么以前会报"网络连接失败"？**

那是浏览器自己堵死了，不是网络的问题。浏览器对同一个域名只开 6 条并发连接，
多出来的请求排在队列里等；而超时是从"创建请求"就开始算的，不是从真正发出去算。
一口气丢 60 个文件进去，排在后面的还没轮到自己就判了超时，于是一片红。

现在文件层面自己限流：同时最多传 3 个，其余显示"排队中"；分片、init、合并
这三步遇到网络错误或 5xx 会自动退避重试（0.8s / 1.6s / 3.2s），
权限不足、配额满这类重试也没用的错误则立刻失败。合并接口做成了幂等的，
重试拿回的还是同一个文件，不会多出一份。

**上传时撞上同名文件会怎样？**

会弹窗问，跟 Windows 复制文件一样三个选项：

| 选项 | 效果 |
| --- | --- |
| 保留两者 | 新文件改名成 `报价单(1).xlsx`，原文件不动。**默认** |
| 替换原文件 | 内容写进原来那个文件，版本号加一。节点 ID 不变，所以原有的权限、分享链接、知识库引用全部继续有效 |
| 跳过这些文件 | 重名的不传，同一批里不重名的照常传 |

几条边界：

- **替换需要对原文件有"编辑"权限**，只有上传权限是不够的（否则等于谁都能
  悄悄改掉别人的文件）。权限不足时"替换"这一项是灰的；真发起了也会被后端挡下来，
  而不是退而求其次偷偷改名——那样你会以为自己已经更新成功了。
- 同名的如果是**文件夹**，文件不能盖掉它，只能改名放进去。
- 上传整个文件夹时，**同名文件夹自动合并**（进到已有的那个里面），
  只有里面的同名文件才会触发上面这个弹窗。
